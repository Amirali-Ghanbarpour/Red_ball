...
let actionHistoryRedo = new Stack();
function onMoveListener(dir) {
   ...
   */ 
  //when press key up,down,left,right that must save reverses
  if(dir === "up")
    actionHistory.push("down");
  if(dir === "down")
    actionHistory.push("up");
  if(dir === "right")
    actionHistory.push("left");
  if(dir === "left")
...
function onUndoAction() {
   ...
   */
  //for save pop dir
  let popd = actionHistory.pop();
  //for change x,y,z
  moveAgent(popd);
  //for save redo that must save reverse
  if(popd === "up")
    actionHistoryRedo.push("down");
  if(popd === "down")
    actionHistoryRedo.push("up");
  if(popd === "right")
    actionHistoryRedo.push("left");
  if(popd === "left")
    actionHistoryRedo.push("right");
}
function onRedoAction(){
  /**
   * This function is called whenever the 'y' key is pressed
   *   and must Redo the last move of the agent
   * 
   * You can use moveAgent function here
   *
   * e.g. the move ro the right direction is implmeneted
   ...
   */
  //for saved pop dir
   let popd = actionHistoryRedo.pop();
   //for change x,y,z
   moveAgent(popd);
   //for save to undo
   onMoveListener(popd);
